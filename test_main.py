...
class TestGQAttention:
    ...
    def test_sdpa_equivalence(self):
        # Build two attention modules with identical state_dict
        # one use_sdpa=False, one use_sdpa=True
        cfg_manual = gqa_cfg(use_sdpa=False)
        cfg_sdpa = gqa_cfg(use_sdpa=True)
        attn_manual = GQAttention(cfg_manual)
        attn_sdpa = GQAttention(cfg_sdpa)
        attn_sdpa.load_state_dict(attn_manual.state_dict())

        x = torch.randn(B, T, cfg_manual.dim)
        mask = torch.triu(torch.full((1, 1, T, T), float("-inf")), diagonal=1)

        with torch.no_grad():
            out_manual = attn_manual(x, self.freqs, mask=mask)
            out_sdpa = attn_sdpa(x, self.freqs, mask=mask)
...
class TestMLAttention:
    ...
    def test_sdpa_equivalence(self):
        cfg_manual = mla_cfg(use_sdpa=False)
        cfg_sdpa = mla_cfg(use_sdpa=True)
        attn_manual = MLAttention(cfg_manual)
        attn_sdpa = MLAttention(cfg_sdpa)
        attn_sdpa.load_state_dict(attn_manual.state_dict())
